...
<head>
    ...
    <title>DocVerify - Setel Ulang Password</title>
    <!-- Google Fonts -->
...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 px-4 sm:px-0">
        <div class="bg-slate-900/80 backdrop-blur-2xl py-8 px-6 shadow-2xl rounded-2xl border border-slate-800/80 sm:px-10">
            <form class="space-y-5" action="/reset-password" method="POST" onsubmit="return validatePasswords()">
                @csrf
                ...
                @if(empty($token))
                    <div class="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-3.5 rounded-xl text-sm text-center font-medium leading-snug flex items-center justify-center gap-2">
                        <i data-lucide="alert-circle" class="w-4 h-4 flex-shrink-0"></i>
                        <span>Token reset tidak valid atau tidak ditemukan di tautan URL Anda.</span>
                    </div>
                @else
                    <div>
    ...
    <script>
        ...
        function validatePasswords() {
            ...
            if (pass !== confirmPass) {
                errorDiv.innerText = 'Password dan konfirmasi password baru tidak sama.';

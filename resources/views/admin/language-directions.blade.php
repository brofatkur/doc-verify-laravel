...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex justify-between items-center border-b border-slate-200 pb-5">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Master Data: Arah Bahasa</h1>
            <p class="text-slate-500 text-sm mt-1">Kelola daftar pasangan arah bahasa resmi yang dilayani oleh Penerjemah IPPTI.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 h-fit">
            <h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
                ...
                <span>Tambah Arah Baru</span>
            </h3>
            <form action="/admin/language-directions" method="POST" class="space-y-4">
                @csrf
                <div class="space-y-2">
                    <label for="name" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Pasangan Bahasa</label>
                    <input
                        type="text"
                        id="name"
                        name="name"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th class="px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Pasangan Arah Bahasa</th>
                        ...
                        @if($directions->isEmpty())
                            <tr>
                                <td colspan="2" class="px-6 py-8 text-center text-slate-400 text-xs">
                                    Belum ada arah bahasa terdaftar.
                                </td>
                            </tr>
                        @else
                            @foreach($directions as $dir)
                                <tr class="hover:bg-slate-50/40 transition-colors duration-150">
                                    <td class="px-6 py-4 font-semibold text-slate-800 text-sm">
                                        <span id="name-display-{{ $dir->id }}">{{ $dir->name }}</span>
                                        <form id="edit-form-{{ $dir->id }}" action="/admin/language-directions/{{ $dir->id }}/update" method="POST" class="hidden flex items-center gap-2">
                                            @csrf
                                            <input
                                                type="text"
                                                name="name"
                                            ...
                                            <button type="button" onclick="cancelEdit('{{ $dir->id }}')" class="p-1 text-rose-600 hover:bg-rose-50 rounded transition cursor-pointer" title="Batal">
                                                <i data-lucide="x" class="w-4 h-4"></i>
                                            </button>
                                        </form>
                                    </td>
                                    <td class="px-6 py-4 text-right flex justify-end gap-2">
                                        <button onclick="enableEdit('{{ $dir->id }}')" id="edit-btn-{{ $dir->id }}" class="text-slate-500 p-1.5 hover:bg-slate-100 rounded-lg transition cursor-pointer" title="Ubah Nama">
                                            <i data-lucide="edit-3" class="w-4 h-4"></i>
                                        </button>
                                        <form action="/admin/language-directions/{{ $dir->id }}/delete" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus arah bahasa ini?')">
                                            @csrf
